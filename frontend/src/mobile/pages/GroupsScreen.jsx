    ...
    <BottomSheet open={open} onClose={onClose} title="New group">
      <div style={{ display: 'grid', gap: 12 }}>
        <div><label className="m-label">Name</label>
          <input className="m-input" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. First-year CS" /></div>
        <div><label className="m-label">Type (optional)</label>
          <input className="m-input" value={form.group_type} onChange={e => setForm(f => ({ ...f, group_type: e.target.value }))} placeholder="cohort / section / year" /></div>
...
function ManageGroupSheet({ group, onClose, onChanged, snack }) {
  ...
  const add = async () => {
    if (!sel) return
    await api.post(`/groups/${group.id}/members/${sel}`); setSel(''); snack('Added'); load(); onChanged && onChanged()
  }
  ...
  return (
    <BottomSheet open={!!group} onClose={onClose} title={`Manage: ${group.name}`}>
      ...
      <div style={{ display: 'grid', gap: 6 }}>
        {(detail?.members || []).map(m => (
          <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0' }}>
            ...
            <button className="m-iconbtn" style={{ width: 34, height: 34, color: 'var(--danger)' }} onClick={() => remove(m.id)}><Trash2 size={15} /></button>
          </div>
        ))}
      </div>
